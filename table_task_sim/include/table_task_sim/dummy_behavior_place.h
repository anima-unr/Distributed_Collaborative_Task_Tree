...
namespace task_net {
	class DummyBehaviorPlace: public Behavior {
	 public:
	  DummyBehaviorPlace();
	  DummyBehaviorPlace(NodeId_t name, NodeList peers, NodeList children,
	    NodeId_t parent,
	    State_t state,
	    std::string object,
	    ROBOT robot_des,
	  ...
	  virtual ~DummyBehaviorPlace();
	  void UpdateActivationPotential();
	  bool ActivationPrecondition();
	  void Place(std::string object, ROBOT robot_des);
	  bool PlaceDone();
	  void Work();
